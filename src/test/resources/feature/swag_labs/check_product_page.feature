...
Функционал: Проверка функционала страницы с продуктами

  Предыстория: Открывается страница
    Пусть пользователь открывает страницу из файла "SwagLab.BaseUrl"
#    Пусть пользователь открывает сайт "https://www.saucedemo.com/"
  ...
  Сценарий: Проверить, что все продукты отображаются на странице
    ...
    * h1(1. Выполняется вход)
    Когда пользователь заполняет поле "user-name" значением из файла "SwagLab.Login"
#    Когда пользователь заполняет поле "user-name" значением "standard_user"
    И он заполняет поле "password" значением из файла "SwagLab.Password"
